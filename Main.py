...
class enemy:
    def __init__(self, enemy_texture, x_coord, y_coord):
        self.__text = pygame.image.load(os.path.join(enemy_texture))
        self.x = x_coord
        self.y = y_coord
    def spawn(self):
        sc.blit(self.__text, (x, y))
...
def enemy_spawn(sig_val = 18):
    try:
        spawn_ind_i = random.randint(1, 25)
        spawn_ind_j = random.choice((0, 1, 24, 25))
        my_map[spawn_ind_i][spawn_ind_j] = 18
    except IndexError:
        spawn_ind_j += 1
...
text_enemy_r = pygame.image.load('1enemy.png')
text_enemy_b = pygame.transform.rotate(text_enemy_r, 90)
text_enemy_l = pygame.transform.rotate(text_enemy_b, 90)
text_enemy_t = pygame.transform.rotate(text_enemy_l, 90)
...
while True:
    if (enemy_timer == 200):
        en_lst.append(enemy('1enemy.png', random.randint(3,23) * width, random.randint(3,23) * height))
        en_lst[en_num].spawn()
        en_num += 1
        enemy_timer = 0
    ...
    if (obj == 2):
        ...
        False
        end_sf = pygame.Surface((WIN_WIDTH + INT_WIDTH,WIN_HEIGHT))
        sc.blit(end_sf, (0, 0))
        end_label = my_font.render('The end!!!!', False, (RED, 0, 0))
        sc.blit(end_label,((WIN_WIDTH + INT_WIDTH) / 2, WIN_HEIGHT / 2))
    enemy_timer += 1
